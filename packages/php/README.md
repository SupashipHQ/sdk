...
A framework-agnostic PHP SDK for Supaship feature flags. It works in Laravel, CodeIgniter, WordPress, and standalone PHP without requiring extra HTTP libraries.
...
```php
SupaClient::fromArray(array $config, ?callable $transport = null)
```

By default the SDK uses native PHP HTTP support (`cURL` if available, otherwise stream transport).

Custom transport signature:

```php
function (string $url, array $headers, string $body, int $timeoutMs): array
```
...
[
  'status' => 200,
  'body' => '{"features":{"new-ui":{"variation":true}}}'
]
```
